...
### Challenges and Mitigations: Handling Background False Positives
During the development phase, a notable challenge arose where the model occasionally misidentified complex environmental structures (such as wooden carport beams or specific backgrounds) as damaged vehicle parts.

* **Dataset Augmentation (Null Images)**: To mitigate this, we introduced "Null" images into the training dataset—photos containing only the empty testing environment (no cars and no bounding boxes). This explicitly taught the YOLOv8 model to recognize and ignore these specific environmental patterns.
* **Inference Filtering**: While the validation confusion matrix may still display minor false positives under the background class, this is not an issue during real-world predictions. The confusion matrix is automatically generated by YOLOv8 at a highly sensitive, low-confidence threshold (~25%) to display all weak guesses. However, in our live production pipeline, the YOLOv8 detection confidence threshold is strictly controlled (adjustable via the UI). This heavy filtering completely drops weak background guesses, ensuring that bounding boxes are only drawn on genuine, high-confidence vehicle damage.
